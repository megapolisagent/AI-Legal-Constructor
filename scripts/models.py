"""Модель данных Deal/Participants — handoff §4. Перенесено без изменений логики
из archive/web_proto/app/models.py — только это (Flask-обвязки здесь не было и раньше)."""
...
def load_deal(path: str | Path) -> Deal:
    """Единственная точка чтения deal.json — используется всеми CLI-инструментами
    ниже (registry_lookup не читает Deal, но check_completeness/generate_package — да)."""
    return Deal.from_dict(json.loads(Path(path).read_text(encoding="utf-8")))
